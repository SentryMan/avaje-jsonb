...
public @interface Json {
   ...
   *
   * <p>By default, the naming used is {@link Naming#Match}
   */
  Naming naming() default Naming.Match;

  /**
   * When {@code @Json.SubType} is used this specifies the name of the property field that holds the
   * type name (discriminator value).
   *
   * <p>This defaults to {@code @type} when unspecified.
   */
   ...
   *
   * <p>We can explicitly use {@code deserialize=true} to include the property in deserialization
   * but not serialization. For example, we might do this on a property that represents a secret
   * like a password.
   *
   * <p>We can explicitly use {@code serialize=true} to include the property in serialization but
   * not deserialization.
   */
  @Retention(CLASS)
  @Target({ElementType.FIELD})
   ...
   *
   * <p>When reading unknown properties from json content these are read and put into this map. When
   * writing json this map is included back into the content.
   *
   * <pre>{@code
   * @Json.Unmapped
  ...
  @interface SubType {
     ...
     *
     * <p>When unspecified this is the short name of the class.
     */
    String name() default "";
  }

  /**
   * Container of all the concrete SubType's that an interface type or abstract type can be
   * represented as.
   */
  @Retention(CLASS)
  @Target({ElementType.TYPE})
