...
public interface JsonWriter extends Closeable, Flushable {
   ...
   *
   * <p>We do this to get access to the underlying generator. For the case when using jackson-core
   * we get access to Jackson JsonGenerator and can then do Jackson specific things like set custom
   * escaping.
   *
   * <pre>{@code
   ...
   * than set per object via {@link #names(PropertyNames)}.
   *
   * <p>This is used by view json generation where all the names are known at the point when the
   * view is created (a sort of flattened nested tree).
   */
   ...
   *
   * <p>This is expected to be called per object after each call to {@link #beginObject()}.
   */
   ...
   *
   * <p>This is generally less efficient than using {@link JsonWriter#names(PropertyNames)} and
   * {@link JsonWriter#name(int)}.
   */
  ...
  /**
   * Write raw content. This is typically used to write new line characters for {@code
   * x-json-stream} content.
   */
  void writeNewLine();
