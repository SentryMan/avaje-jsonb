 ...
 * will use Jackson core JsonParser and JsonGenerator to do the underlying parsing and generation.
 *
 * <pre>{@code
...
public interface Jsonb {
   ...
   *
   * <p>We can register JsonAdapter's to use for specific types before building and returning the
   * Jsonb instance to use.
   *
   * <p>Note that JsonAdapter's that are generated are automatically registered via service loading
   * so there is no need to explicitly register those generated JsonAdapters.
   *
   * <pre>{@code
   * Jsonb jsonb = Jsonb.builder()
   *   .serializeNulls(true)
   ...
   *
   * <p>This is a convenience method for {@code jsonb.type(Object.class).toJsonPretty(any) }
   *
   ...
   *
   * <p>This is a convenience method for {@code jsonb.type(Object.class).toJsonBytes(any) }
   */
   ...
   *
   * <p>This is a convenience method for {@code jsonb.type(Object.class).toJson(any, writer) }
   */
   ...
   *
   * <p>We can use <code>type(Object.class)</code> when we don't know the specific type that is
   * being written toJson or read fromJson.
   *
   * <p>
   *
   * <h3>Object toJson()</h3>
   ...
   * }</pre>
   *
   * <p>When using <code>Object.class</code> and writing <code>toJson()</code> then the underlying
   * JsonAdapter is determined dynamically based on the type of the object value passed in.
   *
   * <p>When using <code>Object.class</code> and reading <code>fromJson()</code> then the java types
   * used in the result are determined dynamically based on the json types being read and the
   * resulting java types are ArrayList, LinkedHashMap, String, boolean, and double.
   */
   ...
   *
   * <p>We can use {@link Types} to obtain common generic types for List, Set, Map, Array etc.
   *
   * <h3>Example</h3>
   *
   * <pre>{@code
   ...
   *
   * <p>This is a helper method that supports returning an inferred generic type.
   *
   ...
   *
   * <p>Provides the option of optimising the writing of json for property names by having them
   * already escaped and encoded rather than as plain strings.
   */
   ...
   *
   * <p>JsonAdapter is generally used by generated code and your application code is expected to use
  ...
  interface Builder {
     ...
     *
     * <p>Default is to not serialise nulls.
     */
     ...
     *
     * <p>Default is to not serialise empty collections.
     */
    Builder serializeEmpty(boolean serializeEmpty);

    /**
     * Set failOnUnknown to true such that an exception is thrown when unknown properties are read
     * in the json content.
     */
    Builder failOnUnknown(boolean failOnUnknown);

    /**
     * Set to true for BigDecimal and BigInteger to serialise as String values rather than number
     * values.
     */
     ...
     *
     * <p>When not set the JsonStreamAdapter is service loaded using {@link AdapterFactory} with a
     * fallback default of using the builtin implementation.
     *
