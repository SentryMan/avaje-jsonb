 ...
 *
 * <p>JsonType provides the main API used to read and write json.
 *
 * <h4>fromJson</h4>
 *
 ...
 * }</pre>
 *
 * <p>Moshi note: JsonType does not exist in Moshi and has been added to provide a slightly nicer
 * API to use than JsonAdapter.
 */
public interface JsonType<T> extends JsonView<T> {
   ...
   *
   * <p>Generally, we should not use this type directly but instead create a {@link JsonReader} and
   * use {@link JsonType#stream(JsonReader)} instead. Then we just use a try-with-resources on the
   * JsonReader and can additionally specify {@link JsonReader#streamArray(boolean)} option.
   *
   * <p>When using this Stream type directly, use a try-with-resources block with the Stream to
   * ensure that any underlying resources are closed.
   *
   * <pre>{@code
  ...
  /**
   * Convert from 'object form' expecting {@code Map<String,Object>} for 'json object' and expecting
   * {@code Collection<?>} for 'json array'.
   *
   ...
   *
   * <p>Use a try-with-resources block with the JsonReader to ensure the underlying resources are
   * closed.
   *
   * <pre>{@code
   ...
   *
   * <p>When using Jackson-core as the underlying parser we should explicitly state that the content
   * is either an ARRAY (with '[' and ']' tokens or not (x-json-stream new line delimited, there are
   * no '[' and ']' tokens).
   *
   * <p>When using the builtin avaje-jsonb parser, it automatically detects and handles both cases
   * (with or without the '[' and ']' tokens). With the Jackson-core parser we need to explicitly
   * state if we are processing
   *
   * <pre>{@code
   * JsonType<MyBean> type =  jsonb.type(MyBean.class);
   *
