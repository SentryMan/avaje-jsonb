...
class JacksonAdapterTest {
  ...
  public static final JsonFactory JSON_FACTORY =
      new JsonFactory()
          ...
          .configure(
              JsonParser.Feature.ALLOW_UNQUOTED_CONTROL_CHARS, true) // required to read line breaks
          .configure(JsonParser.Feature.ALLOW_TRAILING_COMMA, true);

  public static final JsonFactory HUMAN_READABLE_JSON_FACTORY =
      JSON_FACTORY.copy().setCharacterEscapes(new HumanReadableCharacterEscapes());

  @Test
  void custom_JsonFactory() {

    JacksonAdapter jacksonAdapter =
        JacksonAdapter.builder().jsonFactory(HUMAN_READABLE_JSON_FACTORY).build();

    Jsonb jsonb = Jsonb.builder().adapter(jacksonAdapter).add(new MyComponent()).build();
